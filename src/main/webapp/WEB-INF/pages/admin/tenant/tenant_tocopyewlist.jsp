...
{
	$("#queryForm").attr("action","${pageContext.request.contextPath }/tenant/tocopyewlist?page="+(new_page_index + 1));
	$("#queryForm").submit();
}
...
<body>
  ...
  <div class="panel-head">
	...
	<div>
		<form id="queryForm" action="${pageContext.request.contextPath }/tenant/tocopyewlist?page=1" method="post">
			...
			<c:otherwise>
				<c:forEach items="${pageinfo.dataList }" var="tenant">
						<tr>
							...
							<td>
							  <a class="button border-blue button-little" href="${pageContext.request.contextPath}/water/add?room_id=${tenant.room_id}">添加水电</a>
							</td>
						</tr>
				</c:forEach>								
			</c:otherwise>
